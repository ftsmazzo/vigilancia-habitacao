...
// Visão geral dos cards de Vigilância
router.get(
  "/overview",
  requireAuth,
  ...
  async (req, res) => {
    ...
    // Pessoas
    const [pessoasRow] = await prisma.$queryRaw`
      WITH fam AS (
        SELECT cod_familiar_fam
        FROM "vw_vig_familias"
        WHERE
          ${unidadeTerritorial} IS NULL
          OR ${unidadeTerritorial} = 'TODOS'
          OR cod_unidade_territorial_fam = ${unidadeTerritorial}
      )
      SELECT
...
router.post(
  ...
  async (req, res) => {
    ...
    try {
      // Usa IF NOT EXISTS para evitar erro se as views já existirem
...
// Lista de unidades territoriais (CRAS)
router.get(
  "/unidades",
  requireAuth,
  requireRole("MASTER", "ADMIN", "VIGILANCIA"),
